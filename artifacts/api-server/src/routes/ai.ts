...
Be specific with search terms and URLs.
...
router.post("/ai/generate-content", async (req, res) => {
  ...
  const typeInstructions: Record<string, string> = {
    ...
    script: `a TikTok video script (15–30 seconds). Include:
...
Return ONLY this JSON:
{
  ...
  try {
    ...
    req.log.error({ text }, "Failed to parse AI content response");
    res.status(500).json({ error: "Failed to parse AI response" });
  }
});
...
router.post("/ai/generate-listing", async (req, res) => {
  ...
  const response = await openai.chat.completions.create({
    ...
    messages: [
      {
        role: "system",
        content: `You are a TikTok Shop listing specialist with a 8% average conversion rate.
...
Product: ${productName}
Description: ${productDescription}${audienceClause}${featuresClause}
...
Return ONLY this JSON:
{
  ...
  try {
    ...
    req.log.error({ text }, "Failed to parse AI listing response");
    res.status(500).json({ error: "Failed to parse AI response" });
  }
});
...
Return ONLY this JSON:
{
  ...
  try {
    ...
    _req.log.error({ text }, "Failed to parse trending niches response");
    res.status(500).json({ error: "Failed to parse AI response" });
  }
});
...
router.post("/ai/product-analysis", async (req, res) => {
  ...
  const priceClause = estimatedCost
    ? ` Sourcing cost: $${estimatedCost}. Selling price: $${estimatedSellingPrice ?? "TBD"}.`
    : "";
...
Product: ${productName}
Description: ${productDescription}${priceClause}
...
Return ONLY this JSON:
{
  ...
  try {
    ...
    req.log.error({ text }, "Failed to parse product analysis response");
    res.status(500).json({ error: "Failed to parse AI response" });
  }
});
